<template>
    <form @submit.prevent="saveEmailFn()">
		...
		<div class="mb-4">
			<Button type="submit" label="Add Email" class="min-w-28" :loading="loading" :disabled="loading || Email?.file?.length > fileLimit" />
		</div>
	</form>
</template>
...
const { errors, storeEmail } = useEmails();
